...
//        tempParam.setValue("ios", forKey: "terminal")
        ...
        print(body)
        reqUrl = reqUrl + "?" + body
        reqUrl = (reqUrl as NSString).addingPercentEscapes(using: String.Encoding.utf8.rawValue)!
        var req = URLRequest(url: URL(string: reqUrl)!)
//        let bodyData = body.data(using: .utf8)
//        req.httpBody = bodyData
        req.httpMethod = "GET"
...
class UploadNetwork: NSObject {
    ...
    public class func request(_ param: [String:String], data: Any, paramName: String, complete: ((_ responseObject:Any) -> Void)?) -> Void {
